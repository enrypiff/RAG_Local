...
def split_text(data):
    ...
    chunks = []
    document_chunks = text_splitter_large.split_documents(data)
    chunks.extend(document_chunks)
    document_chunks = text_splitter_small.split_documents(data)
    chunks.extend(document_chunks)
...
def save_to_chroma(chunks: 'list[Document]'):
    ...
    # OllamaEmbeddings() is used for embedding the documents.
    # The database is persisted in the directory specified by CHROMA_PATH.
    db = Chroma.from_documents(
        chunks, OllamaEmbeddings(model="mistral"), persist_directory=CHROMA_PATH
    )
